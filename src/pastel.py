...
MACRO_DECL='PASTEL_COMPILE'
MACRO_REG=r'^%(macro)s\((.*),\"(.*)\"\);' % {'macro' : MACRO_DECL}

BUNDLE_STRUCT_DECL='PastelBundle'

GENERATED_WARNING='// File generated by pastel.py - DO NOT MODIFY'
...
#ifndef %(macro)s
#define %(macro)s(name, path) extern %(struct)s name
#endif
""" % {
    'macro' : MACRO_DECL,
    'struct': BUNDLE_STRUCT_DECL,
...
def parse_options_and_args(args):
    options, arguments = getopt.getopt(args, 's:t:', ['generate_header', 'verbose'])
...
def write_symbol(symbol_name, symbol_data, target_file):
    ...
    target_file.write("%(struct)s %(name)s = {\"%(name)s\", %(name)s_data, %(data_size)d};\n" % {'name' : symbol_name, 'struct': BUNDLE_STRUCT_DECL, 'data_size': len(symbol_data)})
...
def parse_source_files(sources, verbose=False):
    symbols = []
    for s in sources:
        matches = [re.findall(MACRO_REG, line.replace(' ', '')) for line in open(s)]
        for m in matches:
            if (len(m) != 0):
                if verbose:
                    print ("Var name: %s" % m[0][0])
                    print ("File name: %s" % m[0][1])
                symbol = {'name': m[0][0], 'data': read_whole_file(m[0][1])}
...
def main():
    ...
    if len(run_config['source_files']) > 0:
        cpp_src = run_config['source_files'][0]
        cpp_sources = run_config['source_files']

        symbols = parse_source_files(cpp_sources, verbose)

        data_file = open(data_file_name, 'w')
        data_file.write(GENERATED_WARNING + "\n")
        data_file.write("#include \"%(header)s\"\n" % {'header': CONFIG_PASTEL_HEADER})

        for s in symbols:
            write_symbol(s['name'], s['data'], data_file)
